#!/bin/bash
#
# release.sh — WindowFinder の配布物（公証済み DMG + appcast）を作る
#
# 前提（初回のみ・あなたの作業）:
...
#   3) Sparkle の EdDSA 秘密鍵が Keychain にある（generate_keys 実行済み・実施済み）
#
# 使い方:
#   ./release.sh 1.0.0
#
...
SCHEME="WindowFinder"
# 証明書作成後、`security find-identity -v -p codesigning` の表示名に置き換える:
DEV_ID_APP="Developer ID Application: SATOSHI MAEDA (Z89YBVR7QV)"
TEAM_ID="Z89YBVR7QV"
NOTARY_PROFILE="WindowFinderNotary"                       # store-credentials の名前
GITHUB_REPO="maeda-niku18/WindowFinder"
# =========================================================================
